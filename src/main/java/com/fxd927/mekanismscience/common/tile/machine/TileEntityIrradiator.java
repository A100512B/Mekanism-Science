...
import com.fxd927.mekanismscience.api.recipes.cache.IrradiatingCachedRecipe;
...
public class TileEntityIrradiator extends TileEntityProgressMachine<IrradiatingRecipe>
        implements ItemChemicalRecipeLookupHandler<Gas, GasStack, IrradiatingRecipe> {
    ...
    @Getter
    private MachineEnergyContainer<TileEntityIrradiator> energyContainer;
    ...
    public TileEntityIrradiator(BlockPos pos, BlockState state) {
        super(MSBlocks.IRRADIATOR, pos, state, TRACKED_ERROR_TYPES, BASE_TICKS_REQUIRED);
    ...
    @Override
    @NotNull
    public IMekanismRecipeTypeProvider<IrradiatingRecipe, ItemChemical<Gas, GasStack, IrradiatingRecipe>> getRecipeType() {
        return MSRecipeType.IRRADIATING;
    }

    @Override
    @Nullable
    public IrradiatingRecipe getRecipe(int cacheIndex) {
        return findFirstRecipe(itemInputHandler, gasInputHandler);
    }

    @Override
    @NotNull
    public CachedRecipe<IrradiatingRecipe> createNewCachedRecipe(@NotNull IrradiatingRecipe recipe, int cacheIndex) {
        return new IrradiatingCachedRecipe(recipe, recheckAllRecipeErrors, itemInputHandler, gasInputHandler, () -> StatUtils.inversePoisson(injectUsage), outputHandler)
