...
import com.fxd927.mekanismscience.common.tile.machine.TileEntityIrradiator;
...
public class IrradiatingRecipeCategory extends BaseRecipeCategory<IrradiatingRecipe> {

    private final GuiGauge<?> inputGauge;
    private final GuiGauge<?> outputGauge;
    private final GuiSlot inputSlot;

    public IrradiatingRecipeCategory(IGuiHelper helper, MekanismJEIRecipeType<IrradiatingRecipe> recipeType) {
        super(helper, recipeType, MSBlocks.IRRADIATOR, 3, 3, 170, 79);
    ...
    @Override
    public void setRecipe(@NotNull IRecipeLayoutBuilder builder, IrradiatingRecipe recipe, @NotNull IFocusGroup focusGroup) {
        ...
        List<GasStack> scaledGases = gasInputs.stream().map(gas -> new GasStack(gas, gas.getAmount() * TileEntityIrradiator.BASE_TICKS_REQUIRED))
                .toList();
